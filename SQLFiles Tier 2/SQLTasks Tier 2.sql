...
the member name. */
SELECT f.name AS Facility, CONCAT(surname, ', ', firstname) AS Name
...
SELECT f.name AS Facility, CONCAT(surname, ', ', firstname) AS Name, 
    CASE
        ...
        WHEN m.memid != 0 AND membercost * slots > 30 THEN membercost * slots
    END AS Cost
...
SELECT f.name AS Facility, CONCAT(surname, ', ', firstname) AS Name, starttime,
    CASE
        WHEN m.memid = 0 AND guestcost * slots > 30 THEN guestcost * slots
        WHEN m.memid != 0 AND membercost * slots > 30 THEN membercost * slots
    END AS Cost
